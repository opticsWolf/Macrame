...
START = "2030-06-01T12:00:00.000000Z"
...
def test_the_stamp_a_write_records_is_the_one_the_clock_was_set_to(db_path, clock):
    ...
    The clock is at 2030 and the wall clock is not, so a stamp in 2030 cannot
    have come from anywhere else — which is what makes this an assertion about
    the transaction-time axis rather than about clock skew.
    """
    ...
    assert len(stamps) == 1
    assert stamps[0].startswith("2030-06-01T12:00:00")
...
def test_advancing_the_clock_moves_the_axis_and_valid_time_stays_put(db_path, clock):
    ...
    assert len(stamps) == 2
    assert stamps[0].startswith("2030-06-01")
    assert stamps[1].startswith("2031-05-31") or stamps[1].startswith("2031-06-01")
...
def test_reopening_a_populated_file_raises_the_clock_to_what_is_stored(db_path):
    ...
    A fake set to 2020 against a ledger stamped 2030 would abort the first
    write on `trg_concepts_monotonic_ra`. `open_tuned` therefore raises the
    clock to the newest stored `recorded_at` first (`Clock::raise_floor`), so
