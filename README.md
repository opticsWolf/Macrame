...
- [Architecture Quick Reference](docs/quickref.md) — API, schema, decisions, performance. **Its version header tracks each release and its body does not**, which is a worse thing for a document to do than being plainly out of date, so it is said here rather than left to be discovered: the schema ladder in §4 is kept current rung by rung and now reads v19, while the API section has not been refreshed since **v0.12.0** / [D-148](docs/architecture/s13-decision-register.md#d-148) and still names paths this crate no longer offers — it carries neither the 0.13.0 wave (D-149…D-169), nor the 0.13.x series toward 1.0 (D-170…[D-212](docs/architecture/s13-decision-register.md#d-212)) with its public-surface changes at D-205…D-208, nor the branching wave (D-213…[D-242](docs/architecture/s13-decision-register.md#d-242)), nor this cycle (D-243…[D-277](docs/architecture/s13-decision-register.md#d-277)). **Refreshing it is not scheduled**, and that is recorded rather than glossed: it is a derived document, and the architecture set below is the one kept true by gates. This README said "v0.9.0 reference" until 0.12.25, which was wrong about its own pointer. Where it disagrees with the architecture set, the architecture set wins — it is the normative one.
...
- [Release notes](docs/releases) — one document per minor, most recently [v0.16.0](docs/releases/v0.16.0.md) (the 0.15.0 review, thirty releases and D-243…[D-273](docs/architecture/s13-decision-register.md#d-273)) and [v0.15.0](docs/releases/v0.15.0.md) (branching). **The version here is cut and the merge to `main` is not made**: the note precedes the merge, as it has since [D-212](docs/architecture/s13-decision-register.md#d-212), and this line will say so until it does
