...
| `wal_autocheckpoint` | `Default` / `Disabled` / `EveryPages(n)`. Disabling it is only correct paired with an explicit `checkpoint()`, and the cost is deferred rather than removed: ~8,400–9,100 frames in 41–45 ms at the end, against ~860 in 5.5–6.2 ms along the way ([D-157](docs/architecture/s13-decision-register.md#d-157)). **On a bulk import the threshold is the dial** (measured, [D-275](docs/architecture/s13-decision-register.md#d-275)): a 16,000-edge random-order import runs 5.0 s at the default, 3.7 s at `EveryPages(10_000)` with a ~43 MB WAL, 3.6 s disabled with a ~554 MB WAL — the recipe, numbers and shape caveat live in `Database::bulk_import`'s docs |
...
| Schema version | **18** |
| Test suite | 797 Rust · 775 with `--no-default-features` · 666 Python (2 skipped) — all green (measured 2026-09-20 at 0.18.0 on one Windows box and **not replicated on CI**: the pull request for `dev/0.18.0` is not open at the cut, so the three-runner replication this line carried at 0.17.0 does not extend to these three numbers, and [D-234](docs/architecture/s13-decision-register.md#d-234) and [D-262](docs/architecture/s13-decision-register.md#d-262) apply to them exactly as they applied to every one-box figure before them. The per-item deltas across the cycle — 769 → 770 → 781 → 794 → 797 — are in the register at [D-279](docs/architecture/s13-decision-register.md#d-279), [D-280](docs/architecture/s13-decision-register.md#d-280) and [D-278](docs/architecture/s13-decision-register.md#d-278) and [D-286](docs/architecture/s13-decision-register.md#d-286), each measured at its own commit. **The last replicated figures are 0.17.0's** — 769 / 747 / 616, measured 2026-09-11 and replicated on all three CI runners at that release commit itself — run `34626309278`: 769 on ubuntu and macOS at attempt 1/3, 769 on Windows at attempt 2/8, the feature-off 747 on ubuntu at attempt 1/3, and the quarantined property arm **793 across 64 targets** at attempt 1/6 on ubuntu and macOS with `crashed-R15` at 6/6 on Windows, which is [D-236](docs/architecture/s13-decision-register.md#d-236)'s read and the reason that step is non-blocking. **Those figures are replicated; the ones this line carried through 0.16.3 were the 0.16.0 release's** — 757 / 736 / 607, run `34155656904` — and the patch releases between carried their own counts in the register ([D-274](docs/architecture/s13-decision-register.md#d-274)…[D-277](docs/architecture/s13-decision-register.md#d-277)) rather than re-arguing this line per release. Read [D-234](docs/architecture/s13-decision-register.md#d-234) and [D-262](docs/architecture/s13-decision-register.md#d-262) before quoting *any earlier* release's version of this line as replicated: D-243 … D-261 shipped with no CI run of any kind, because the branch filter did not reach `dev/**` and the draft pull request the policy relies on was never opened). `metrics` is a **default** feature since 0.12.11, so the first figure is a plain `cargo test`; the second is the same suite with the counters compiled out, and the **22-test gap** is `actor_metrics_tests` (14, the whole target), `src/metrics.rs`'s five unit tests, one gated test in `checkpoint_tests`, one gated test in `bulk_deferred_tests` (D-277's exemption assertion; D-276's exemption assertions are gated *inside* `vector_tests`, whole-test count unchanged), and the doc-test on `Database::metrics` — enumerated at 0.14.23 by diffing `cargo test -- --list` against the same list under `--no-default-features`, which is the only way this sentence stays true as the metrics target grows. **The second figure is a CI gate since 0.14.24** ([D-241](docs/architecture/s13-decision-register.md#d-241)): the feature-off suite is *run* on ubuntu, not merely compiled, because until then nothing could falsify the number this line publishes. Regenerate rather than trust this line: `python scripts/run_rust_suite.py`, and `python scripts/run_rust_suite.py --no-default-features` for the second — a bare `cargo test --no-default-features` stops at the first R15 crash and reports the partial count as the total |
...
- [Architecture Quick Reference](docs/quickref.md) — API, schema, decisions, performance. **Its version header tracks each release**, and its schema ladder §4 is kept current rung by rung (it reads **v19**), as is the chunk-size table and the newest decisions: the pre-0.16 drift that made this paragraph necessary was corrected rung by rung through 0.16.x, and the 0.16.x decisions sit in §9's table ([D-274](docs/architecture/s13-decision-register.md#d-274)…[D-277](docs/architecture/s13-decision-register.md#d-277)). **The stale part is named rather than glossed**: the API section's long prose still dates to **v0.12.0** / [D-148](docs/architecture/s13-decision-register.md#d-148) — it does not carry the 0.13.0 wave (D-149…D-169), the 0.13.x series toward 1.0 (D-170…[D-212](docs/architecture/s13-decision-register.md#d-212)), or the branching wave (D-213…[D-242](docs/architecture/s13-decision-register.md#d-242)) in prose, only in the schema ladder and the decision rows — and that prose refresh is still not scheduled: it is a derived document, and the architecture set below is the one kept true by gates. This README said "v0.9.0 reference" until 0.12.25, which was wrong about its own pointer. Where it disagrees with the architecture set, the architecture set wins — it is the normative one.
- [Python bindings](docs/architecture/s14-python-bindings.md) — §14: async→sync boundary, error tree, stubs
- [Decision register](docs/architecture/s13-decision-register.md) — D-001…D-277 with rationale
- [Release notes](docs/releases) — one document per minor, most recently [v0.18.0](docs/releases/v0.18.0.md) (the flexible ledger: [D-278](docs/architecture/s13-decision-register.md#d-278)…[D-286](docs/architecture/s13-decision-register.md#d-286), `concepts.extra`, the KV sidecar, the relaxed edge-kind charset, opt-in attributes on the subgraph loader, and two stated forward incompatibilities) and [v0.17.0](docs/releases/v0.17.0.md) (the comparison cycle: D-274…[D-277](docs/architecture/s13-decision-register.md#d-277), the bulk path's defect and the two opt-in recipes). **The version here is cut and the merge to `main` is not made**: the note precedes the merge, as it has since [D-212](docs/architecture/s13-decision-register.md#d-212), and this line will say so until it does
