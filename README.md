...
- **Every error is typed** — 49 exception classes under `MacrameError` (50 including the base), of which seven are intermediate groups for catching sets — `IntegrityError`, `ValidationError`, `VectorError`, `TemporalError`, `WriterError`, `BudgetError`, `BranchError` — leaving 42 leaves. `BranchError` is 0.15.0's, over the six refusals a lineage can raise. A `DbError` variant added without a class is caught by `binding_parity_tests`, which **runs** rather than compiles: `DbError` is `#[non_exhaustive]` since 0.13.34, so the mapping's wildcard arm is mandatory and the compiler no longer checks completeness ([D-207](docs/architecture/s13-decision-register.md#d-207)).
...
- [Decision register](docs/architecture/s13-decision-register.md) — D-001…D-241 with rationale
