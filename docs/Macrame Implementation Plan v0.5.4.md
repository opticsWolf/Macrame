...
| 5.4 | `load_subgraph_with` — the missing `edge_types` / `min_weight` filter | open |
...
> Worth naming: this wave has so far *removed* an unreachable branch, *removed* a set that served it, and *declined* to add two mechanisms. The output is a smaller crate and three facts that were not written down. That counts.
