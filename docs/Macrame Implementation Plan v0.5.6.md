...
Sequenced from the post-Wave-4 review. Two items delivered; four remain.
...
| 5.5 | Consolidate the three "storage permits what the API refuses" statements into §4 | open |
| 5.6 | `write_annotations` rename; three clippy warnings | open |
...
> Worth naming: this wave has so far *removed* an unreachable branch, *removed* a set that served it, *declined* to add two mechanisms, and added one method that turned up two accounting bugs older than itself. The output is a smaller crate and five facts that were not written down. That counts.
