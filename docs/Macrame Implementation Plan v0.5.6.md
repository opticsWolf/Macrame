...
Sequenced from the post-Wave-4 review. Four items delivered; two remain.
...
> Worth naming: this wave has so far *removed* an unreachable branch, *removed* a set that served it, *declined* to add two mechanisms, added one method that turned up two accounting bugs older than itself, and turned a documentation merge into a finding. The output is a smaller crate and six facts that were not written down. That counts.
