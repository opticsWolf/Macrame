...
*The mechanism is a schedule, not a habit.* libFuzzer is nightly-only and CI can compile these but not run them, which is where the `--fuzz-check` gate ([D-263](architecture/s13-decision-register.md#d-263)) stops. So a weekly scheduled workflow on a **pinned** nightly runs each target for 30 s. Pinned because a floating nightly eventually goes red for a toolchain reason rather than a crash, and a job that cries wolf is [D-236](architecture/s13-decision-register.md#d-236)'s shape. And a crash must produce something a person receives — the artifact uploaded and an issue opened — because *remember to look at the Actions tab* is the same failure as *remember to run it*, with more YAML.
