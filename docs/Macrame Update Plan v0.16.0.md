...
**Shipped as 0.15.19, [D-261](architecture/s13-decision-register.md#d-261).** Eleven items, one release, a row each in the register. Ten are what the review called them. **C-13 was not**, and finding that out cost nothing more than building the state instead of reading the note.

*The item that was not a nit.* C-13 reads as a tidiness note: `COLD_SCHEMA` runs before `BEGIN IMMEDIATE`, so a failed session can leave a cold file with schema and no horizon row, and *"nothing downstream is wrong"*. Probed (`examples/cold_file_reach_probe.rs`): **`ATTACH` creates the file**, before any DDL, so the DDL's placement is not what puts a file on disk and the suggested fix would not have removed one. The second suggestion — treat a missing `archive_horizon` row as no archive — would declare `archive_branch`'s real archives absent, since [D-230](architecture/s13-decision-register.md#d-230) writes no such row on purpose. And something downstream *is* wrong: the leftover is 0 bytes, so every `reconstruct` below the newest hot stamp takes the cold arm and fails with a raw `no such table: cold.transaction_log`. Reachable with nothing failing unexpectedly — `rehydrate` on a ledger that had never been archived attached, met `no such table: cold.concepts`, returned that, and left the file. **A database's entire history, unreadable, because a caller asked to rehydrate something that was never archived.** Three parts: an empty file is not an archive (which heals a database already in that state, at open, with nothing to delete by hand), `rehydrate` does not attach when there is nothing to attach, and the schema pass moves inside the transaction so that *non-empty* means *complete*. Pinned by `rehydrating_without_an_archive_leaves_no_archive_behind`, found under mutation.

*The snapshot chain is now checked by something that runs.* [D-092]'s `verify_snapshot_chain` folds from genesis and its own rustdoc names scheduling it as the open problem, so composition had never been checked outside a test. `verify_last_link` re-derives snapshot *n* from *n−1* — one anchored delta — and the cadence runs it after every anchor, logging divergence at `warn`. It reports and never repairs, under Doctrine VI. Its limit is asserted rather than argued: `the_link_check_does_not_see_a_defect_older_than_one_link` damages *n−1*, lets *n* compose onto it, and requires the link check to agree while the genesis check diverges — which is the executable reason the genesis check stays.

*Two more that were not quite as filed.* C-20 says `abort_kind` keys on libSQL's prefix; it matched with `contains`, and `examples/abort_code_probe.rs` shows the engine returns `SqliteFailure(1811, …)` carrying the crate's string verbatim with no prefix at all — the gate on the extended code is still worth having, for a different reason, written down. C-15's `NOT LIKE '%_shadow'` was not merely an unescaped wildcard: it hid any real model whose name ends in *shadow* from its own registry, and the arm is deleted rather than escaped because the `embeddings_` prefix already excludes every libSQL internal.

The rest are what they say: the shadow swap recreates DDL from two named lists with completeness gates instead of `contains("links_current")` [C-12]; `save_and_prune`'s join arm raises `SnapshotWriteFailed` [C-14]; the hybrid rank lookup is a `HashMap` [C-16]; the anchored fold's comment stops repeating the mechanism [D-049](architecture/s13-decision-register.md#d-049) disproved [C-17]; the cadence skips its two aggregates when the actor's committed-turn count has not moved, by counting turns rather than publishing a `seq_id` the actor does not know [C-19]; the closed-interval arm's cost is written into its rustdoc [C-21]; and `rehydrate` reads and deletes by chunk in the caller's order [C-22]. Surface **1,757 → 1,761**; suites **745** and **726** Rust, **592** Python; schema unchanged at **v17**.

### Ongoing, not a release (A-4, A-5, A-6)

`temporal/archive.rs` split into its four modules (A-4); the rest of A-5 as the three parts below. **A-6 is closed**: measured at 0.15.22 ([D-264](architecture/s13-decision-register.md#d-264)) and remedied at 0.15.23 ([D-265](architecture/s13-decision-register.md#d-265)) — the mechanism it named was gone, the hang it predicted was real in a different place, and the *closing* flag on this list is one half of what shipped. None of the remainder gates 0.16.0.

#### A-5 is four bullets in three different states, and only one of them is the hard part

The review files A-5 as one finding. It is not. `dev/**` in CI **shipped at 0.15.20** ([D-262](architecture/s13-decision-register.md#d-262)) and is struck from the list below. Of the three that remain, the fuzz targets are half a day, the exempt-kind visibility is a reporting gap rather than the blindness A-5 describes, and the lineage generator is the finding — the one thing on this list that would have caught a defect that shipped.
...
*The mechanism already exists, and the paragraph this replaces got that wrong.* It said libFuzzer is nightly-only and CI can compile but not run these, so a weekly schedule was needed. **`ci.yml` has had a `fuzz` job since 0.13.14** — nightly toolchain, `cargo install cargo-fuzz --locked`, three snapshot targets at 90 s each, crash artifacts uploaded on failure — running on every push and every pull request. So the four targets are four more steps in a job that is already there, at 30 s each because a pure function over a short string saturates its input space far faster than a container format does. A schedule would be *weaker* than what exists: weekly instead of per-push, and a second place for the toolchain and the corpus wiring to drift.
...
**Part 2 — the lineage property generator (one release; the real item). Shipped as 0.15.26, [D-268](architecture/s13-decision-register.md#d-268) — and point 6 came back with more than it asked for: the generator reproduced [D-229](architecture/s13-decision-register.md#d-229) against a reverted clause in about a second, then failed on its first run against a *clean* tree with a defect nobody knew about, [D-269](architecture/s13-decision-register.md#d-269).**

Every property this codebase checks by generation is checked on a graph with one branch. `doctrine_property_tests` generates single-lineage histories from five ops; `integrity_property_tests` has an archive-shaped strategy and no fork. [D-229](architecture/s13-decision-register.md#d-229) — an archive that matched supersession across lineages, so one `archive` made the trunk stop reaching a node it believed in — was found by hand, and A-5's claim is that a generator would have found it.

1. **Ops gain lineage**: `Fork`, `AssertOn`, `RetireOn`, `Archive`, `ArchiveBranch`, alongside the five that exist. Public API only, rejections ignored — the existing file's rule, and for its reason: a doctrine that holds only when nobody uses the API is not a doctrine.
2. **The bias is the design, not a detail.** `AssertOn` and `RetireOn` must reuse *the trunk's existing interval keys* far more often than random ones. [D-229](architecture/s13-decision-register.md#d-229)'s first symptom needed a branch writing at an ancestor's exact key; a uniform generator reaches that almost never. This is the whole difference between a generator that would have found the bug and one that merely looks like it would.
3. **Four properties, checked after every op**: an archive never changes what *any* lineage reaches (this is the one D-229 fails, in both of its shapes); `audit_current` reports zero; `reconstruct_on(ts, branch)` agrees with the fold for every live branch; archiving one branch leaves every other branch's reach alone.
4. **Few cases, long histories.** Each case opens a database and concurrent opens are [R15](architecture/s13-decision-register.md#d-147)'s trigger, which is why the existing doctrine properties run 24 and 8 cases rather than 200. More ops per open, not more opens.
5. **Promotion is automatic, and the obvious version of it does not work.** `.proptest-regressions` replays every seed it has ever found before generating a novel case — but it belongs to the property binary, and that binary is quarantined: it runs, reports and blocks nothing ([D-236](architecture/s13-decision-register.md#d-236)). The automation exists and lands where the exit code is discarded. So a **second, non-quarantined binary that only replays**: it reads the committed seeds, runs each through the same op-runner, generates nothing, and is not feature-gated — deterministic, a bounded number of opens, and inside the suite whose CRASH retry already handles R15. A found bug then stays fixed with nobody copying anything. The minimised case is *also* written by hand once it is understood — not for the gate, which the replay binary already is, but because a file of opaque integers tells the next reader that something was once wrong here and nothing about what.
6. **It must be shown to catch the bug it was written for.** Revert [D-229](architecture/s13-decision-register.md#d-229)'s `branch_id` equality clause in a scratch tree and confirm the generator fails, with a named case, inside its budget. A generator that has never caught its own motivating defect is decoration, and this codebase's habit is to verify a gate by injection rather than assert it. If it does not fire, the fix is the bias, not the budget — and the negative result is worth recording either way.
7. **And the generator itself is checked for reach**, on both sides. A separate test draws a few thousand histories from the strategy with no database at all and asserts each interesting shape appears: a branch asserting at a key the trunk already wrote, a branch retiring an inherited edge, an archive with a live fork, an `archive_branch` on a branch that has written. A bias is a probability and probabilities drift when the op set changes; a generator that silently stops reaching its own bias is worse than none, because everything passes and the passing is what gets pointed at. It fails on **too much** as well as too little: if nearly every history is the interesting shape, the ordinary path has stopped being tested.

**Part 3 — cost visibility on the budget-exempt kinds (one release). Shipped as 0.15.28, [D-271](architecture/s13-decision-register.md#d-271) — and the criterion group falsified two of its own four expected shapes, one of them the sentence in `connection.rs` it was written to turn into a number. Building it also found, in a path it does not measure, a branch write costing about a thousand times the same write on the trunk; that is recorded there and repaired in its own release.**

A-5 says the archive's `rebuild_within` is invisible because `Archive` is budget-exempt. Reading the code, that is no longer where the gap is. **The data already exists**: `record_hold` writes turns, total, longest and the full histogram for every kind, and the only thing it skips for an exempt one is the `over_budget` counter. **The gap is the report.** `budget_violations()` filters to kinds with `over_budget > 0`, which for an exempt kind is zero by construction — so a dashboard sees nothing, permanently, for exactly the operations whose cost nobody bounds. The data is collected and the reporting path drops it.

So: a companion `exempt_costs()` returning those kinds with turns, mean, longest and histogram; and a criterion group `exempt_kinds` with a scaling arm, driven by `MACRAME_BENCH_SCALE`, for each exempt kind that has none — `RebuildCurrent`, `Checkpoint`, `ArchiveBranch` and `ShadowSwap`, where `Archive` has had the flat-slice arm since [D-245](architecture/s13-decision-register.md#d-245) and `Rehydrate` has two. Each arm states its expected shape, flat or linear, in its own comment, so a number that changes shape is legible without a baseline file. **Not gated** — [D-055](architecture/s13-decision-register.md#d-055) stands, these are seen and not enforced, and `perf_claim_tests` does not grow an assertion here.

#### The branch-write repair, which is what Part 3 found and did not do

**Shipped as 0.15.29, [D-272](architecture/s13-decision-register.md#d-272).** Not an A-5 item: A-5 asked for cost *visibility*, the instrument built for it found a cost, and this is the cost being fixed. It is written down here rather than in the register alone because [D-271](architecture/s13-decision-register.md#d-271) deferred it on a stated condition — *"a change to the write path's overlap guard is a change to the hot path of every assertion in the crate; it wants its own release, its own before-and-after on both shapes, and its own argument about which of the two costs is the one being fixed"* — and those three things are the whole content of this item.

**The symptom.** A 200-edge batch asserted on a fork of a 2,000-edge trunk cost **68 s** against the same batch's **26 ms** on the trunk. Not a constant factor: at an 8,000-edge trunk the guard alone cost **5.4 s per asserted row** against 268 ms at 2,000 — twenty times the cost for four times the trunk.

**Which of the two costs.** D-271 named two suspects in one plan and could not separate them: the log arm of `links_cut_cte` reading `SEARCH transaction_log USING INDEX idx_txlog_fold_partition (table_name=?)`, and an automatic index SQLite builds each execution. `examples/branch_write_guard_probe.rs` separates them by running five spellings of the same statement side by side, and the answer is **the join order alone**. The automatic index survives the repair untouched and costs nothing measurable; the arm that was scanning the whole links log stops.

**What the plan actually was**, which is worse than the entry recorded and is the reason the number is quadratic rather than merely large:
...
`transaction_log` drives and `churned` is inlined as the inner loop — so for every one of the log's `links` rows, the whole of that lineage's `links_current` is walked with **one** column bound, having lost both the edge key and the `recorded_at > cutoff` bound that `idx_lc_lineage_cut` exists to serve. That is O(log rows × lineage rows) per asserted row, and `write_edges_atomic` runs it once per row of the batch.

**The repair is four words.** `FROM churned k CROSS JOIN transaction_log`, which is SQLite's documented way of saying *do not reorder this*. It is sound because `links_current`'s primary key is `(source_id, target_id, edge_type, valid_from, branch_id)`, so `churned` is unique on the `(entity_id, branch_id)` pair the join matches: the join is one-to-many in exactly one direction and the window's `ROW_NUMBER` sees the same input either way. With `churned` driving, the log becomes a **three**-column seek — `(table_name=? AND entity_id=? AND branch_id=?)` — which is a better access path than the `idx_txlog_entity (entity_id=?)` this arm had before 0.15.12, so **[D-254](architecture/s13-decision-register.md#d-254)'s index is not the villain and is not touched.** It was being used with one column bound instead of three.

**Both shapes, and the third one nobody asked about.** The guard is one of five relations `plan::lower` builds, and `links_cut_cte` is in every branched *current-belief read* as well — unkeyed there, so the churned set is the whole of it rather than one edge. `examples/resolved_read_probe.rs` measures that side through the public API on a fixture with deliberate post-fork churn, and the branched read was carrying the same defect: **13.6 ms → 1.3 ms** at fork depth 8, and it stops growing with depth at all. The trunk control and the transaction-time read do not move, which is what says the measurement is of this change.

**The gate is a plan pin, not a millisecond.** `the_log_arm_is_driven_by_the_churned_set` sits beside `the_guard_seeks_the_edge_key_on_every_shape` in `lineage.rs`'s own tests, asserts that the log arm binds `entity_id`, and is verified by injection — reverting the four words must make it red. A timing assertion would be [D-055](architecture/s13-decision-register.md#d-055)'s mistake and a number on this machine.
...
**Read as evidence rather than ticked, in `docs/releases/v0.16.0.md`.** Seven met, two restated and then met, and the tenth met two releases late: criterion 10 was unreachable from this branch until [D-262](architecture/s13-decision-register.md#d-262) widened CI's branch filter, and the first run it made possible came back red in three jobs ([D-263](architecture/s13-decision-register.md#d-263)). Criterion 10's own wording — `--attempts 3` — is one of the things that run refuted: three is enough on Linux and macOS and is not enough on Windows, where the suite passed on attempt 7 of 8. The number in `ci.yml` is per-OS now, and this sentence is left as written because a criterion is a claim with a date on it.
