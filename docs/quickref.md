...
**v0.18.0 · A Bitemporal Graph Ledger on libSQL**
...
**Two things to know before using it.** `concepts` is **narrowed, and needs no resolving** (0.15.18, [D-260](architecture/s13-decision-register.md#d-260)): an invisible lineage contributes nothing and an ancestor's post-cutoff writes are cut, and that is all a concept needs, because **two visible lineages cannot both hold one concept id**. A concept id is globally unique and the schema refuses the collision by name, so a branch inherits its parent's concepts and cannot restate them — the deferred overlay design ([D-214](architecture/s13-decision-register.md#d-214)) is what would change that, and would need a nearest-lineage rule for concepts along with it. And a forked read **cannot use snapshots at all**: a snapshot has no `recorded_at` left in it for a cutoff to compare against, so this folds from genesis and costs ~3 ms flat whatever the fork depth — about 4× `reconstruct` where snapshots are configured, and ~1.15× where they are not. An **unforked** database pays none of it and delegates to `reconstruct`, snapshots and all.
...
impl Database {
    ...
    pub async fn bulk_import_deferred(&self, edges) -> BulkResult<usize>  // 0.16.3, D-277
...
**The suite is not the whole gate, and one gap shipped** (0.11.0, [D-144](architecture/s13-decision-register.md#d-144)). CI also runs `cargo doc --no-deps --features metrics` under `RUSTDOCFLAGS: -D warnings`, and rustdoc's checks are **not tests**: a broken intra-doc link is a warning that flag escalates, so no `cargo test` run and no assertion this project could write will see it. 0.10.0 was tagged and released with one. Run it locally the same way as the suite: `python scripts/run_rust_suite.py --doc-check --features metrics` (`--docs` is the same gate under its original name). **It is a separate command and no suite run implies it** — 0.18.0 shipped two broken links past a green 794/772/663 because the suite was run and this was not, which is [D-144](architecture/s13-decision-register.md#d-144) recurring against a contributor who had the gate and did not reach for it. Nothing is retried — rustdoc opens no database, so a failure there is always real. **Run it that way, not as bare `cargo doc`** (0.15.21, [D-263](architecture/s13-decision-register.md#d-263)): without the flag the same nine errors arrive as nine warning lines, and a warning count that holds steady across releases reads as a baseline while the gate is red underneath it.

**The fuzz crate is a second workspace, and nothing else compiles it** (0.15.21, [D-263](architecture/s13-decision-register.md#d-263)). `fuzz/` is not a workspace member, so `cargo check --all-targets` at the root does not reach it and neither does the suite; it went nine releases without compiling after [D-255](architecture/s13-decision-register.md#d-255)'s `#[non_exhaustive]` sweep, because a struct expression for such a type is refused only outside the defining crate. `python scripts/run_rust_suite.py --fuzz-check` compiles it on stable in seconds. Run it after any change to a public type's shape.
...
| **`bulk_import_deferred` skips the mirror, restores before rebuilding** | D-277 | 2.05× at 16k random-pair (5.08 s → 2.48 s incl. the 191 ms chunked rebuild); the ledger is complete at every instant of the window — the log mirror and the single-open guard stay up — so what lags is the Doctrine VI projection, never the past; the shipped path never has a window, which is why it is a signature |
