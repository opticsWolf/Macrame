...
**W4.3 result (run, 0.10.0):** `ATTACH` is **allowed** and the attachment **inherits the read-only flags** — `INSERT` and `CREATE TABLE` into it are refused, after `PRAGMA query_only = OFF`, and `ATTACH` of a nonexistent path is `SQLITE_CANTOPEN` with no file created. No fifth *write* row was needed and no new finding was promoted, so **W4.1 stays scoped as written**. The permission table gained the `ATTACH` rows anyway, in both columns, because an untested edge on a published boundary claim is the defect W1 was about. Recorded as [D-137](architecture/s13-decision-register.md#d-137), including the one thing `ATTACH` *does* widen: reading, over the filesystem rather than over this database.

**W4.1 + W4.2 result (done, 0.10.0):** the exposure was measured before the fix and after it, at width 48 — **7 bad runs in 18** unserialised (2 × `0xC0000005`, 5 returned SQLite errors) and **0 in 18** with the mutex, via a new `tests_py/probes/r15_diagnostic_path.py`. Both call sites now go through one `PyDatabase::diagnostic_rows` helper rather than duplicating the lock. `Database::diagnostic_conn` gets the paragraph and the numbers, not the lock, for the reason W4.2 names. Two prose locations in §14 asserted the opposite ("opening per call is also the R15-safe shape") and are corrected. [D-138](architecture/s13-decision-register.md#d-138).
