...
**Stated so it can fail.** `tests/storage_boundary_tests.rs` asserts each of the three from the other side: that raw SQL writes the overlapping pair, that `assert_edge` accepts the negative weight, and that `read_conn()` refuses a write where `raw()` does not. A passing suite there is not a virtue — it is a fact about where the checks currently are. If a later migration closes one with a trigger or a `CHECK`, that test fails, and this section is corrected rather than left describing a limit that no longer exists. Prose about what is *not* enforced is the kind that stops being true silently, which is the whole reason this cycle exists.
