...
| 3 | Edge weights are **non-negative** | `load_subgraph`, `NegativeEdgeWeight` | the schema *and the write API* — `links.weight` is a bare `REAL NOT NULL` with no `CHECK`, and `EdgeAssertion::normalized` validates ids, edge type and timestamps but not weight | [D-039](s13-decision-register.md#d-039), [§5.4](s5-modules.md#54-graphsubgraphrs-and-graphalgorithmsrs--native-in-memory-analytics) |
...
**The third is not the same shape as the other two, and consolidating must not pretend it is.** For 1 and 2 the supported write path enforces the rule and the gap opens only for a writer who goes around it. For 3 the supported write path *accepts* the value: `assert_edge` with a weight of −1.5 commits, and the refusal arrives later, when something asks for a subgraph. So a file this crate wrote by itself, with no other client involved, can hold a row the crate declines to read back. That is the honest statement, and it is why the invariant is worth listing beside the other two rather than filed under graph loading.

**Why none of the three is closed here.** Each has its own argument in its own decision, and they converge: 1 would add a second index probe to every insert on the path [D-059](s13-decision-register.md#d-059) had just finished making fast, to constrain callers already going through the actor; 2 is not enforceable at this layer at all, since the file is reachable by any SQLite client and making `raw()` private would buy the appearance of a guarantee rather than the guarantee; 3 wants a `CHECK` on `links.weight`, which is a [D-036](s13-decision-register.md#d-036) change to frozen core and is not taken unilaterally — whether it *should* exist in the schema is the one of the three still genuinely open.

**Stated so it can fail.** `tests/storage_boundary_tests.rs` asserts each of the three from the other side: that raw SQL writes the overlapping pair, that `assert_edge` accepts the negative weight, and that `read_conn()` refuses a write where `raw()` does not. It also pins the *opposite* for NaN — that all three write doors refuse it — because that is the claim this section got wrong once, and a claim about what the storage layer already enforces needs a tripwire just as much as a claim about what it does not. A passing suite there is not a virtue — it is a fact about where the checks currently are. If a later migration closes one with a trigger or a `CHECK`, that test fails, and this section is corrected rather than left describing a limit that no longer exists. Prose about what is *not* enforced is the kind that stops being true silently, which is the whole reason this cycle exists.
