...
<a id="doctrine-viii"></a>VIII. Fidelity is a parameter, never a silent default. Queries that mix time axes say so in their signatures. as_of(ts) means valid time under current belief and returns exactly that; reconstruct(ts) means belief as of ts and replays exactly that. The gap between the two — retroactive assertions made after ts — is documented, pinned by tests, and surfaced at the type level. A caller should never receive yesterday's graph with today's text and assume it is history.
