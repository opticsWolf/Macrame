...
*Correction 3 — "chunk size trades latency for throughput" is false on half the paths.* Two of the four are **superlinear** in chunk size. On the edge path marginal cost rises from ~11 µs per row at n=10 to ~103 µs at n=1,000; on the embedding path from ~35 µs to ~151 µs. For those two, the old 1,000-row chunk was simultaneously the worst latency *and* the worst throughput available: 1,000 edges cost 88.5 ms as one chunk against ~27 ms as eleven 90-row chunks, a 3.3× speedup for a 37× latency reduction. Shrinking those chunks is not a trade and never was. The trade is real only on the two linear paths (concepts ~23 µs/row, annotations ~2.5 µs/row), where a smaller chunk does cost throughput — a few percent — and is paid deliberately.

The mechanism behind the superlinearity is **not identified** and is recorded as open in [D-058](s13-decision-register.md#d-058). Both affected paths maintain a secondary structure per row (the two ledger triggers; the DiskANN index), and the shape is consistent with dirty pages spilling to the WAL mid-transaction, but neither has been confirmed and the re-derivation does not depend on which is true.
```rust
// shape only — see Appendix A for normative signatures
