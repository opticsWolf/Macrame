...
>
> **Retention is the part that is now behind.** This section specifies "the last five plus one daily for thirty days" and the implementation is still newest-five-flat. That divergence cost nothing while snapshots were written once per shutdown; a cadence writes them continuously, so five anchors can span minutes and every older instant falls back to a full fold. It is a cost regression only — no answer changes — but it is the next thing here.
