...
`diagnostic_conn()` opens the file with `SQLITE_OPEN_READ_ONLY`. That is an OS-level boundary: `PRAGMA query_only = OFF` is accepted and changes nothing, and the next write fails with `readonly`. `tests/diagnostic_conn_tests.rs` asserts the pair in *both* directions, because asserting only that the diagnostic connection refuses a write would pass equally for a second `query_only` connection — what distinguishes them is what happens after the pragma comes off.

**This paragraph said "returns a connection *of the caller's own*" until 0.15.15, and it stopped being true in 0.15.14.** [D-256](s13-decision-register.md#d-256) made it one connection per `Database`, opened on first use: `connect()` was 51.5 µs of an 82.7 µs call, and it was also this path's exposure to [R15](s11-s12-milestones-and-risks.md#r15) — 3 bad runs in 30 with a connection per call, 0 in 30 with one. D-256 corrected the three documents that named `build()` as the open and did not reach this one or the quick reference, which is how a release that was *about* documents disagreeing with the code left two more behind ([D-257](s13-decision-register.md#d-257)).

The second need it serves is independence **from the crate's own readers**, which is the half of D-091 that survives. `read_conn()` is shared with every traversal and fold in the process, so a long reporting query there competes with them; a diagnostic connection is not that connection. Independence between *diagnostic callers* is what D-256 traded away and [D-257](s13-decision-register.md#d-257) partly bought back: since 0.15.15 the shared connection is scrubbed on the way in — an open transaction rolled back, a connection carrying temp objects or an `ATTACH` discarded, the crate's own pragmas restated — with one documented residue, a pragma the crate does not itself set. `Database::scrub_diagnostic_conn()` runs the same scrub for a caller who is finished with the clone it was handed.
...
**What the read-only flag does not bound (0.15.16, [D-258](s13-decision-register.md#d-258)).** `SQLITE_OPEN_READ_ONLY` bounds writes to the file. It does not bound everything reachable through a connection, because not everything a SQL statement can change belongs to the file or even to the connection. `PRAGMA hard_heap_limit = 1` sets a limit inside the SQLite library, one per process, and after it the whole process fails every write, read, `checkpoint()`, `close()` and `open()` with `out of memory` — measured in `tests_py/probes/diagnostic_global_pragmas.py`, and measured identically against the 0.15.13 shape where every call minted its own connection, so it is not a consequence of the sharing [D-256](s13-decision-register.md#d-256) introduced. Six other global-looking pragmas were measured and are harmless. This sits with the `ATTACH` warning in `diagnostic_conn`'s rustdoc — a connection that takes arbitrary SQL is bounded by what SQL can do, not by what the flags say — and it is documented rather than blocked, because refusing statements by matching their text is guesswork that would read as a guarantee.
...
The `seq_id DESC` ordering resolves same-timestamp entries within a single transaction correctly by construction, and since 0.15.12 `idx_txlog_fold_partition` lets SQLite stream each partition without a global sort — **which this sentence claimed of `idx_txlog_entity` for ten releases and which was not true of it**; see the paragraph below. Where an anchored fold is used, the `seq_id > :anchor` predicate is an inequality, which correctly skips any gaps left by rolled-back transactions ([D-024](s13-decision-register.md#d-024) — see the [§4.3](s4-schema.md#43-the-transaction-log) monotonicity note). The fold routes rows on `table_name`, deserializes payloads with `serde_json` (branching on the payload version field, and raising `PayloadVersion` for anything newer than the crate understands), populates `ReplayCorrupt` with the actual offending `seq_id` rather than a placeholder, and builds an adjacency index as it goes — so the resulting `MaterializedState` answers node, edge and neighbour queries with the shape of the live `Database`. Callers do not need to know whether they are querying the present or the past. The fold is a read: it runs on `read_conn` and never touches the actor — which is what decided the shape of [D-249](s13-decision-register.md#d-249): a verdict every recorded-time read needs, on a connection the write actor does not own, cannot be cached in the actor and cannot be cached in a temp table either, because `read_conn` is `query_only` and refuses to create one. It is kept in the database.

**The fold sorted its whole input, and the index that stops it is not the one the sentence above used to name (0.15.12, W15.2, [D-254](s13-decision-register.md#d-254), review C-4, schema v17).** `idx_txlog_entity` is a single-column index and this window partitions on **three** — `(table_name, entity_id, branch_id)`, `table_name` leading deliberately, because a concept's `entity_id` is its id and a link's is the synthetic `source|target|type|valid_from` and the namespaces are not disjoint. So the planner seeked `idx_txlog_time` for the `recorded_at` bound and then built a temp B-tree over everything it selected: `USE TEMP B-TREE FOR ORDER BY`, on every reconstruction, since the fold existed. `idx_txlog_fold_partition` is those three columns plus `seq_id DESC`, and **the `DESC` is the whole of the effect** — the ascending form of the identical columns is chosen by the planner and still sorts (`USE TEMP B-TREE FOR RIGHT PART OF ORDER BY`). Measured at 30,000 log rows over 12,000 partitions: the fold's own SQL 64.2 → 46.2 ms, `reconstruct()` end to end **99.5 → 72.6 ms**, for +8.2% on the file and **+5.0%** on single-edge assertions. The covering form is 1.18× faster again and is refused: it duplicates `payload` and the file grows 51%, on the one table that only grows.

The review that asked for this asked for `(entity_id, branch_id)`, and that index is **worse than no index at all** — wide enough that the planner takes it and drops the `recorded_at` seek, not ordered enough to supply the window, so the plan becomes a full scan *plus* the same sort. It is the [D-089](s13-decision-register.md#d-089) case the rule exists for, arriving as a review item rather than as an idea, and the probe is `examples/txlog_fold_index_probe.rs`.

**The archived fold needed the cold file indexed too, and the reason is the union.** SQLite compiles the `UNION ALL` above as a `MERGE`, which sorts **each side independently** — so the hot index alone removes one of the two sorts. Measured with 30,000 rows on each side: 127.2 ms with neither, 110.1 with the hot side only, **96.3 with both**. The cold index is applied with `upgrade_cold_lineage` rather than in `COLD_SCHEMA` ([§5.7](s5-modules.md#57-temporalarchivers--cold-storage)), because it names `branch_id` and that list runs against files that may predate the column.
...
**A concept can outlive its lineage, and the refusal says which (0.15.11, W15.1, [D-253](s13-decision-register.md#d-253), review C-3).** The two arms above were built four releases apart and neither knew about the other. The branch arm takes the `branches` row — that is the whole of what makes a fold omitting the lineage correct — and rehydration returns a concept still carrying the `branch_id` it was minted on, into a column that `REFERENCES branches(branch_id)` with foreign keys on. So for one input the two disagree, and the disagreement arrived as `ErrorKind::Engine` reading `FOREIGN KEY constraint failed`: it named neither the concept the caller asked for nor the lineage that is missing, and blamed `concepts`, the table being written, rather than `branches`, the one with the missing row. It is now [`DbError::BranchArchived`](s13-decision-register.md#d-253) naming both, classified under `ErrorKind::Branch`, raised before the insert rather than by it. Reinstating the `branches` row inside the rehydrate — the other repair the review offered — would have made the call succeed and is refused: [D-230](s13-decision-register.md#d-230)'s stance is that an archived lineage is *forgotten*, and bringing the name back here would register a lineage with none of its links without anyone having decided to. The remedy is `fork`, and the cold file holds what it needs — `cold.branches` keeps the parent, the fork point and the instant it left.
