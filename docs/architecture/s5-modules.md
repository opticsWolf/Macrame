...
*And a floor, which is a knowing violation of the bound.* Feedback alone converges to whatever meets 3 ms, and on a populated `links` table there is no such size: per-row cost grows with the table ([D-142](s13-decision-register.md#d-142)), so the fixed ~0.8 ms of Correction 2 keeps taking a larger share until the loop reaches chunks of one or two rows and the import stops finishing. `CHUNK_FLOOR = 35` stops it, measured on the [D-088](s13-decision-register.md#d-088) matrix at 8,000 edges: **≈4.1 ms populated and ≈1.6 ms empty**, a fixed-cost share of ~19% and ~51%. Both rows are published rather than one averaged pair, because they disagree and the disagreement is the point — the same size is comfortably inside the bound on one fixture and 1.1 ms outside it on the other.

That miss is **steady state, not a transient on the way down**, and it is defended by the argument the bound answers to rather than by the bound itself. The first paragraph of this section derives 3 ms from a 60 Hz frame: an interactive assertion waits for the chunk in flight and then runs its own ≤ 5 ms write. At the floor that is 4.1 + 5 = **9.1 ms against a 16.7 ms frame** — still inside it, with room. Halving the floor would recover about 1 ms of that and cost the import roughly a third of its throughput ([D-058](s13-decision-register.md#d-058)). The constant carries its measurement in a comment so the trade can be re-run rather than re-argued.
