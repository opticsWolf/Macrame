...
The fold partitions on `entity_id` alone, which is sound only because `table_name = 'links'` is in the `WHERE`: the discriminator is applied by the filter instead of by the partition, so the concept/link collision defect W is about cannot arise. The four folds in `replay.rs` carry it in the partition instead, and the difference is deliberate.
...
**The snapshot file is a versioned container (0.5.4, [D-043](s13-decision-register.md#d-043); v3 in 0.13.12, [D-185](s13-decision-register.md#d-185)).** A fixed header precedes the payload, written uncompressed and checked before anything is decompressed:
