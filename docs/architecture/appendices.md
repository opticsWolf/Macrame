...
| `db.hybrid_search(text, &v).rrf_k(60)` | **Does not exist.** `reciprocal_rank_fusion` is a pure function over two rank lists; nothing runs the keyword half or fuses them, and there is no FTS5 table in the schema. |
...
Two of these were gaps rather than naming differences. The first is closed; the second is live:

- ~~**The vector write path.**~~ **Closed in 0.5.4 ([D-048](s13-decision-register.md#d-048)).** `Database::register_model` and `Database::upsert_embeddings` route through the actor; the free functions remain for callers already holding a connection. Left in this table rather than deleted, because A.2 exists to record what was promised and whether it arrived — and this one arrived under a different signature than the sketch proposed.
- **Hybrid search.** [§5.6](s5-modules.md#56-temporalas_ofrs--valid-time-queries-and-attribute-hydration) and [§9](s6-s10-flows-to-dependencies.md#9-performance-budgets) both budget for it. Only the fusion arithmetic exists.
