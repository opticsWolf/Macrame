...
**Locally that installation does not happen, and the difference is a whole class of silent failure.** The suite runs with `python/` on `PYTHONPATH`, so it imports `python/macrame/__init__.py` and, beside it, `python/macrame/_macrame.pyd` — a build artifact checked out into the source tree that *nothing writes as a side effect of anything*. `cargo build` writes `target/release/`, which no import path looks at. So the extension goes stale without saying so, and the failure has three recorded instances: at 0.12.17 a `site-packages` copy five releases behind won over the editable path entry and the suite went **green** against it, and at 0.15.10 and 0.15.11 a version bump with no rebuild went red as `assert '0.15.9' == '0.15.10'` — a stale binary that reads as a broken manifest. The green one is the shape worth designing against: `test_packaging`'s version check compares the installed wheel to the installed binding, so when both are old and agree, it passes.

`python scripts/build_python_ext.py` is now the dev loop — `cargo build --release -p macrame-py --features extension-module`, then the copy, then an import in a fresh interpreter to confirm the version. It exists as a script rather than as a line in a README because the obvious alternative is wrong in a way that succeeds: run from `bindings/python/`, which has no `pyproject.toml`, maturin falls back to the `Cargo.toml` and builds a `macrame_py` distribution shipping only `_macrame` — no package, no stubs, nothing written to `python/macrame/` — then prints `Installed macrame-py-0.15.11` and exits 0.

`run_suite.py` refuses to start against an extension that is missing, that disagrees with `bindings/python/Cargo.toml`, or whose mtime predates the newest `.rs` under either crate, and names that script in the message. The mtime question is the one the artifact cannot answer about itself, and so the only one that closes 0.12.17's hole. It is asked **only when the imported package is the in-tree one**: under `pip install .` the extension was built from these sources in one step and there is no second copy to drift, so CI sees the check stand aside. `test_packaging` carries the same predicate under the same guard, so that a bare `pytest tests_py/test_x.py` — which skips the gate — cannot be quietly wrong either.
